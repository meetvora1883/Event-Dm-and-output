...
const { Client, GatewayIntentBits, EmbedBuilder, ActionRowBuilder, StringSelectMenuBuilder, SlashCommandBuilder, ModalBuilder, TextInputBuilder, TextInputStyle } = require('discord.js');
...
const attendanceSchema = new mongoose.Schema({
  ...
  username: String,
  timestamp: { type: Date, default: Date.now },
  actionCount: Number  // Stores kill count, parachute count, etc.
});
...
// Handle modal submissions for action counts
client.on('interactionCreate', async interaction => {
  ...
  try {
    ...
    if (!eventData) {
      await interaction.reply({ content: '❌ Event data not found', ephemeral: true });
      return;
    }
