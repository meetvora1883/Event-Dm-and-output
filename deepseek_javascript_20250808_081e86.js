...
const CONFIG = {
  ...
  HIGH_COMMAND_ROLE_IDS: process.env.HIGH_COMMAND_ROLE_IDS ? process.env.HIGH_COMMAND_ROLE_IDS.split(',') : ['1368991091868700774', '1368991334513508533', '1368991334513508534'], // Added 3 high command roles
  ...
  OUTPUT_CHANNEL_NAME: process.env.OUTPUT_CHANNEL_NAME || 'event-attendance' // Added output channel name
};
...
const EVENT_NAMES = [
  ...
  "𝕃𝕖𝕗𝕥𝕠𝕧𝕖𝕣 ℂ𝕠𝕞𝕡𝕠𝕟𝕖𝕟𝕥𝕤", "ℝ𝕒𝕥𝕚𝕟𝕘 𝔹𝕒𝕥𝕥𝕝𝕖", "𝔸𝕚𝕣𝕔𝕣𝕒𝕗𝕥 ℂ𝕒𝕣𝕣𝕚𝕖𝕣 (𝕠𝕟 𝕊𝕦𝕟𝕕𝕒�𝕪)",
...
async function processAttendance(eventName, date, users, sourceMessage, commandChannel, eventBonus) {
  try {
    ...
    for (const { user, nickname, username, tag, displayName, isHighCommand } of memberInfo) {
      try {
        if (isHighCommand) {
          highCommandMembers.push(displayName || username);
          logCommand(sourceMessage.author, 'processAttendance', 'DEBUG', `Skipped bonus for high command: ${tag}`);
          continue;
        }
    ...
    // Prepare high command message if any
    let highCommandMessage = '';
    if (highCommandMembers.length > 0) {
      highCommandMessage = `\n\n⚠️ High Command Members (no bonus awarded):\n${highCommandMembers.map(name => `• ${name}`).join('\n')}`;
    }
    ...
    const outputContent = `**${eventName} - Attendance**\n**Date:** ${date}\n**Bonus Awarded:** $${eventBonus} per participant\n**Total Bonus Awarded:** $${totalBonusAwarded}\n\n` +
      memberInfo
        .filter(({ isHighCommand }) => !isHighCommand)
    ...
    if (highCommandMembers.length > 0) {
      replyMessage += `\n\n⚠️ Note: ${highCommandMembers.length} high command members did not receive bonuses`;
    }
    ...
    logCommand(sourceMessage.author, 'processAttendance', 'SUCCESS', `Processed ${users.size} users (${highCommandMembers.length} high command skipped)`);
