...
async function handlePaidBonus(interaction) {
  ...
  // Send DM to the user
  try {
    const outstanding = userBonus.total - newPaid;
    const dmEmbed = new EmbedBuilder()
      .setColor(0x00FF00)
      .setTitle('💰 Bonus Paid')
  ...
  try {
    ...
    const memberInfo = await Promise.all(
      Array.from(users.values()).map(async user => {
        const member = await sourceMessage.guild.members.fetch(user.id).catch(() => null);
        return {
          user,
          nickname: member?.nickname || null,
          username: member?.user.username || user.username,
          tag: member?.user.tag || user.tag
        };
      })
    );
    ...
    for (const { user, nickname, username, tag } of memberInfo) {
      try {
        ...
        logCommand(sourceMessage.author, 'processAttendance', 'WARN', `Failed DM to ${tag}: ${error.message}`);
      }
    }
    ...
    const outputContent = `**${eventName} - Attendance**\n**Date:** ${date}\n**Bonus Awarded:** $${eventBonus} per participant\n**Total Bonus Awarded:** $${totalBonusAwarded}\n\n` +
      memberInfo.map(({ user, nickname, username }) => 
