...
const CONFIG = {
  ...
  HIGH_COMMAND_ROLE_IDS: process.env.HIGH_COMMAND_ROLE_IDS ? process.env.HIGH_COMMAND_ROLE_IDS.split(',') : ['1368991091868700774', '1368991334513508533', '1368991334513508534'],
...
const EVENT_NAMES = [
  ...
  "𝕃𝕖𝕗𝕥𝕠𝕧𝕖𝕣 ℂ𝕠𝕞𝕡𝕠𝕟𝕖𝕟𝕥𝕤", "ℝ𝕒𝕥𝕚𝕟𝕘 𝔹𝕒𝕥𝕥𝕝𝕖", "𝔸𝕚𝕣𝕔𝕣𝕒𝕗𝕥 ℂ𝕒𝕣𝕣�𝕚𝕖𝕣 (𝕠𝕟 𝕊𝕦𝕟𝕕𝕒𝕪)",
...
async function processAttendance(eventName, date, users, sourceMessage, commandChannel, eventBonus) {
  try {
    ...
    const outputContent = `**${eventName} - Attendance**\n**Date:** ${date}\n**Bonus Awarded:** $${eventBonus} per participant\n**Total Bonus Awarded:** $${totalBonusAwarded}\n\n` +
      `✅ Eligible Participants (${eligibleMembers.length}):\n` +
    ...
    let replyMessage = `✅ Attendance recorded for ${users.size} users!\n` +
                       `💰 $${totalBonusAwarded} in bonuses awarded to ${eligibleMembers.length} eligible members\n` +
