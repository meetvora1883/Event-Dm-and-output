...
client.on('interactionCreate', async interaction => {
  ...
  try {
    ...
    if (interaction.commandName === 'attendance') {
      ...
      // Initial reply for the attendance command
    ...
    // Ensure interaction is replied to, even on error
...
async function handleListBonus(interaction) {
  ...
  try {
    // Defer reply to prevent "Unknown interaction" if fetching takes time
    ...
    for (const userBonus of sortedBonuses) {
      try {
        ...
        if (currentEmbed.data.fields?.length >= 5) { // Limit fields per embed
        ...
        // Add a placeholder for unfetchable members
        ...
        currentEmbed.addFields({
          name: `Unknown User (${userBonus.userId})`,
          value: `Total: $${userBonus.total}\nPaid: $${userBonus.paid}\nOutstanding: $${Math.max(0, userBonus.total - userBonus.paid)} (User not found)`,
          inline: true
        });
      }
    }
    ...
    // Send the first embed as the initial reply
    await interaction.editReply({
    ...
    for (let i = 1; i < embeds.length; i++) {
      await interaction.followUp({
        embeds: [embeds[i]],
        flags: 64 // Keep follow-up ephemeral
      });
    }
    ...
    // Ensure reply is sent even if deferred
...
client.on('interactionCreate', async interaction => {
  ...
  try {
    // Defer the update to acknowledge the select menu interaction immediately
    ...
    // Edit the original reply to ask for bonus amount
    await interaction.editReply({
    ...
    amountCollector.on('collect', async amountMessage => {
      try {
        ...
        if (isNaN(bonusAmount)) {
          ...
          return; // Stop processing this collector if invalid
        }
        ...
        // Edit the original interaction reply to show date options
        await interaction.editReply({
          content: `✅ Event: **${eventName}**\n💰 Bonus: **$${bonusAmount}**\n\n📅 Choose date option:`,
          components: [row]
        });

        await amountMessage.delete().catch(() => {}); // Clean up user's message
      } catch (error) {
        logCommand(interaction.user, '/attendance [amount]', 'FAIL', error.message);
        // Attempt to inform user about the error
        await interaction.followUp({
          content: '❌ An error occurred while processing the bonus amount. Please try again.',
          flags: 64
        }).catch(() => {});
      }
    });
    ...
    amountCollector.on('end', collected => {
      if (collected.size === 0) {
        // If no message was collected, the user timed out or didn't respond
        interaction.editReply({
          content: '⏰ You took too long to provide the bonus amount. Please start the `/attendance` command again.',
          components: []
        }).catch(() => {});
        logCommand(interaction.user, '/attendance [amount]', 'WARN', 'User timed out on bonus amount input');
      }
    });

  } catch (error) {
    logCommand(interaction.user, '/attendance [select]', 'FAIL', error.message);
    // Ensure the original interaction is replied to if an error occurs early
    if (!interaction.replied && !interaction.deferred) {
      await interaction.reply({
        content: '❌ An unexpected error occurred during event selection. Please try again.',
        flags: 64
      }).catch(() => {});
    } else if (interaction.deferred) {
      await interaction.editReply({
...
client.on('interactionCreate', async interaction => {
  ...
  try {
    ...
    const eventMatch = interaction.message.content.match(/✅ Event: \*\*(.*?)\*\*\n💰 Bonus: \*\*\$(.*?)\*\*/);
    if (!eventMatch) {
      logCommand(interaction.user, '/attendance [date]', 'WARN', 'Could not parse event details from message');
      await interaction.editReply({
        content: '❌ An error occurred. Could not retrieve event details. Please start over.',
        components: []
      }).catch(() => {});
      return;
    }
      ...
      case 'custom':
        ...
        dateCollector.on('collect', async dateMessage => {
          try {
            ...
            await interaction.followUp({
              content: '❌ An error occurred while processing the custom date. Please try again.',
              flags: 64
            }).catch(() => {});
          }
        });
        ...
        dateCollector.on('end', collected => {
          if (collected.size === 0) {
            interaction.editReply({
              content: '⏰ You took too long to provide the custom date. Please start the `/attendance` command again.',
              components: []
            }).catch(() => {});
            logCommand(interaction.user, '/attendance [date]', 'WARN', 'User timed out on custom date input');
          }
        });
        
        return; // Exit switch as custom date handling is asynchronous
      default:
        // Should not happen if options are well-defined
        logCommand(interaction.user, '/attendance [date]', 'WARN', `Unknown date option: ${dateOption}`);
        await interaction.editReply({
          content: '❌ An unknown date option was selected. Please try again.',
          components: []
        }).catch(() => {});
        return;
    }
    ...
    logCommand(interaction.user, '/attendance [date]', 'FAIL', error.message);
    await interaction.editReply({
      content: '❌ An unexpected error occurred during date selection. Please try again.',
      components: []
    }).catch(() => {});
  }
});
...
async function askForTime(interaction, eventName, eventBonus, date) {
  try {
    // Edit the original interaction reply to ask for time
    await interaction.editReply({
      ...
      try {
        ...
        mentionCollector.on('collect', async mentionMessage => {
          try {
            ...
            // Process attendance and send final reply
            await processAttendance(eventName, date, timeInput, users, interaction, interaction.channel, eventBonus);
            await mentionMessage.delete().catch(() => {});
          } catch (error) {
            logCommand(interaction.user, '/attendance [process]', 'FAIL', error.message);
            await interaction.followUp({
              content: '❌ An error occurred while processing participants. Please try again.',
              flags: 64
            }).catch(() => {});
          }
        });
        ...
        mentionCollector.on('end', collected => {
          if (collected.size === 0) {
            interaction.editReply({
              content: '⏰ You took too long to mention participants. Please start the `/attendance` command again.',
              components: []
            }).catch(() => {});
            logCommand(interaction.user, '/attendance [mentions]', 'WARN', 'User timed out on mentions input');
          }
        });

        await timeMessage.delete().catch(() => {}); // Clean up user's message
      } catch (error) {
        logCommand(interaction.user, '/attendance [time]', 'FAIL', error.message);
        await interaction.followUp({
          content: '❌ An error occurred while processing the time. Please try again.',
          flags: 64
        }).catch(() => {});
      }
    });
    ...
    timeCollector.on('end', collected => {
      if (collected.size === 0) {
        interaction.editReply({
          content: '⏰ You took too long to provide the time. Please start the `/attendance` command again.',
          components: []
        }).catch(() => {});
        logCommand(interaction.user, '/attendance [time]', 'WARN', 'User timed out on time input');
      }
    });
    ...
    logCommand(interaction.user, '/attendance [time]', 'FAIL', error.message);
    await interaction.editReply({
      content: '❌ An unexpected error occurred while asking for time. Please try again.',
      components: []
    }).catch(() => {});
  }
}
...
async function processAttendance(eventName, date, time, users, originalInteraction, commandChannel, eventBonus) {
  try {
    // Validate channels
    const outputChannel = originalInteraction.guild.channels.cache.get(CONFIG.OUTPUT_CHANNEL_ID);
    
    if (!outputChannel) {
      const errorMsg = 'Output channel not found';
      logCommand(originalInteraction.user, 'processAttendance', 'FAIL', errorMsg);
      await originalInteraction.followUp({ // Use followUp as original interaction is already replied/deferred
    ...
    const memberInfo = await Promise.all(
      Array.from(users.values()).map(async user => {
        const member = await originalInteraction.guild.members.fetch(user.id);
        const isHighCmd = isHighCommand(member);
        const info = await getMemberDisplayInfo(originalInteraction.guild, user.id);
        return {
          user,
          ...info,
    ...
    for (const { user, nickname, username, tag, displayName, isHighCommand } of memberInfo) {
      try {
        if (isHighCommand) {
          ...
          logCommand(originalInteraction.user, 'processAttendance', 'DEBUG', `High command DM sent to ${tag}`);
          continue;
        }
        ...
        if (eventBonus > 0) {
          ...
          await updateUserBonus(user.id, {
            ...
            history: [{
              ...
              date: new Date().toISOString(),
              by: originalInteraction.user.id
            }]
          });
          ...
          if (dmResult.success) {
            logCommand(originalInteraction.user, 'processAttendance', 'DEBUG', `DM sent to ${tag}`);
          } else {
            logCommand(originalInteraction.user, 'processAttendance', 'WARN', `Failed DM to ${tag}: ${dmResult.error}`);
          }
        } else {
        ...
        logCommand(originalInteraction.user, 'processAttendance', 'ERROR', `Error processing ${tag}: ${error.message}`);
        if (isHighCommand) {
          highCommandMembers.push({
            name: username,
            dmStatus: '❌'
    ...
    // Edit the original interaction reply with the final summary
    await originalInteraction.editReply({
      content: replyMessage,
      components: [] // Remove any remaining components
    });
    logCommand(originalInteraction.user, 'processAttendance', 'SUCCESS', 
      `Processed ${users.size} users (${eligibleMembers.length} eligible, ${highCommandMembers.length} high command)`);

  } catch (error) {
    logCommand(originalInteraction.user, 'processAttendance', 'FAIL', error.message);
    console.error(`[${new Date().toISOString()}] 🛑 Processing Error:`, error.stack);
    // Attempt to send a follow-up error message if the original interaction was already replied/deferred
    await originalInteraction.followUp({
      content: '❌ An error occurred while processing attendance. Please check the console for details.',
      flags: 64
    }).catch(() => {});
  }
}
