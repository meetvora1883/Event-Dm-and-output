...
const { Client, GatewayIntentBits, EmbedBuilder, ActionRowBuilder, StringSelectMenuBuilder, ApplicationCommandOptionType, InteractionResponse } = require('discord.js');
  ...
  try {
    ...
    if (interaction.commandName === 'outstanding_bonus_dm') {
      ...
      try {
        ...
        await interaction.editReply({ content: '✅ Check your DMs for bonus summary!', flags: InteractionResponse.FLAGS.EPHEMERAL });
      } catch (error) {
        console.error('Outstanding DM Error:', error);
        await interaction.editReply({ content: '❌ Failed to send DM. Please ensure your DMs are open.', flags: InteractionResponse.FLAGS.EPHEMERAL });
      }
      return;
    }
